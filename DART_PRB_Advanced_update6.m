function DART_PRB_Advanced_update6()
...
function [dl_slicing_ratios, ul_slicing_ratios, new_rl_state] = updateWithAdaptiveAllocation(rl_state, dl_service_demands_sum, ul_service_demands_sum, round, current_utilization)
    ...
    utilization_threshold = 0.70; % Target utilization threshold
    
    % Base fairness weight selection based on utilization
    if current_utilization < 0.65
        % When utilization is very low, prioritize utilization over fairness
        fairness_weight = 0.25; % Reduced fairness weight to boost utilization
    elseif current_utilization < utilization_threshold
        % When utilization is below threshold but not critically low
        fairness_weight = 0.35; % Moderate fairness weight
    else
        % When utilization is good, we can afford to prioritize fairness
        fairness_weight = 0.45; % Higher fairness weight
    end
    
    % Calculate weighted ratios with adaptive fairness weight
    ...
    temp_ratios = [v2x_ratio; embb_ratio; mmtc_ratio];
    current_fairness = calculateJainsFairnessIndex(temp_ratios);
    
    % If fairness is still too low, apply fairness correction
    if current_fairness < 0.75
        % Find min and max allocation
        [min_val, min_idx] = min(temp_ratios);
        [max_val, max_idx] = max(temp_ratios);
        
        % Redistribute a small amount (up to 5%)
        redistribution = min(0.05, (max_val - min_val) * 0.3);
        temp_ratios(max_idx) = temp_ratios(max_idx) - redistribution;
        temp_ratios(min_idx) = temp_ratios(min_idx) + redistribution;
        
        v2x_ratio = temp_ratios(1);
        embb_ratio = temp_ratios(2);
        mmtc_ratio = temp_ratios(3);
    end
    
    % Calculate adaptive minimum guarantees
    v2x_min = 0.15;  
    embb_min = 0.15; 
    mmtc_min = 0.15;
    
    % Apply adaptive resource balancing if utilization is too low
    if current_utilization < 0.65 && isfield(new_rl_state, 'dl_utilization_history')
        % Calculate average service utilization from history
        if isfield(dl_service_demands_sum, 'v2x_utilization') && ...
           isfield(dl_service_demands_sum, 'eMBB_utilization') && ...
           isfield(dl_service_demands_sum, 'mMTC_utilization')
           
            v2x_utilization = dl_service_demands_sum.v2x_utilization;
            embb_utilization = dl_service_demands_sum.eMBB_utilization;
            mmtc_utilization = dl_service_demands_sum.mMTC_utilization;
            
            % Boost allocation to services with higher utilization
            utilization_boost_weight = 0.2;
            total_utilization = v2x_utilization + embb_utilization + mmtc_utilization;
            
            if total_utilization > 0
                v2x_ratio = (1 - utilization_boost_weight) * v2x_ratio + utilization_boost_weight * (v2x_utilization / total_utilization);
                embb_ratio = (1 - utilization_boost_weight) * embb_ratio + utilization_boost_weight * (embb_utilization / total_utilization);
                mmtc_ratio = (1 - utilization_boost_weight) * mmtc_ratio + utilization_boost_weight * (mmtc_utilization / total_utilization);
            end
        end
    end
    ...
    if isfield(new_rl_state, 'traffic_history') && ...
       ...
       ~isempty(new_rl_state.traffic_history.v2x)
        ...
        if total_avg > 0
            ...
            v2x_min = max(min_floor, min(0.3, 0.5 * avg_v2x / total_avg)); 
            embb_min = max(min_floor, min(0.3, 0.5 * avg_embb / total_avg)); 
            mmtc_min = max(min_floor, min(0.3, 0.5 * avg_mmtc / total_avg));
        end
    end
    ...
    % Apply maximum caps (reduced when utilization is low)
    if current_utilization < 0.65
        max_cap = 0.55;
    else
        max_cap = 0.45;
    end
    v2x_ratio = min(max_cap, v2x_ratio);
    embb_ratio = min(max_cap, embb_ratio);
    mmtc_ratio = min(max_cap, mmtc_ratio);
    
    % Adjust based on outage history to maintain QoS
    if isfield(new_rl_state, 'outage_history')
        outage_adjustment = 0.03;
    ...
    if ~isfield(new_rl_state, 'fairness_history')
        new_rl_state.fairness_history = [];
        new_rl_state.fairness_target = 0.80; 
    end
    
    temp_ratios = [v2x_ratio; embb_ratio; mmtc_ratio];
    current_fairness = calculateJainsFairnessIndex(temp_ratios);
    new_rl_state.fairness_history = [new_rl_state.fairness_history; current_fairness];
    
    % If we have enough history, check for long-term fairness
    if length(new_rl_state.fairness_history) > 5
        avg_fairness = mean(new_rl_state.fairness_history(end-min(5,length(new_rl_state.fairness_history)-1):end));
        
        % If average fairness is below target, boost fairness more aggressively
        if avg_fairness < new_rl_state.fairness_target && current_utilization > 0.65
            % Adjust ratios toward equal allocation (only if utilization permits)
            v2x_ratio = 0.7 * v2x_ratio + 0.3 * (1/3);
            embb_ratio = 0.7 * embb_ratio + 0.3 * (1/3);
            mmtc_ratio = 0.7 * mmtc_ratio + 0.3 * (1/3);
        end
    end
