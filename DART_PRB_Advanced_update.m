function DART_PRB_Advanced_update()
...
function DART_PRB_Enhanced_Simulation()
    ...
    for alg_idx = 1:num_algorithms
        ...
        if strcmp(algorithms{alg_idx}, 'Advanced DART-PRB')
            ...
            % Initialize outage history tracking for SLA awareness
            rl_state.outage_history = struct('v2x', 0.01, 'embb', 0.05, 'mmtc', 0.08);
            rl_state.learned_adjustment = struct('v2x', 0, 'embb', 0, 'mmtc', 0);
        ...
        for round = 1:num_rounds
            ...
            for drop = 1:drops_per_round
                ...
                try
                    if strcmp(algorithms{alg_idx}, 'Advanced DART-PRB')
                        % Use optimized resource allocation with controlled utilization
                        [dl_allocation_v2x, dl_allocation_embb, dl_allocation_mmtc] = optimizeResourceAllocation(network, dl_slicing_ratios);
                        % Handle UL allocation separately (can use the same approach with UL ratios)
                        [ul_allocation_v2x, ul_allocation_embb, ul_allocation_mmtc] = optimizeResourceAllocation(network, ul_slicing_ratios);
                        ...
                        % % First apply energy-efficient resource allocation
                        % [dl_energy_allocation, ul_energy_allocation, power_allocation] = allocateEnergyEfficientResources(network, dl_slicing_ratios, ul_slicing_ratios, 'dl');
                        % 
                        % % Then proceed with hierarchical allocation but with energy efficiency awareness
                        ...
                        % 
                        % % Initialize total allocation tracking
                        % total_allocation = zeros(C.N_RB, 1);
                        % 
                        % % First give priority to V2X allocation (most critical)
                        % for i = 1:C.N_RB
                        %     if dl_allocation_v2x(i) == 1 || dl_energy_allocation(i, 1) > 0
                        %         dl_allocation_v2x(i) = 1;
                        %         total_allocation(i) = 1;
                        %     else
                        %         dl_allocation_v2x(i) = 0;
                        %     end
                        % end
                        % 
                        % % Then eMBB allocation
                        % for i = 1:C.N_RB
                        %     if total_allocation(i) == 0 && (dl_allocation_embb(i) == 1 || dl_energy_allocation(i, 2) > 0)
                        ...
                        % end
                        % 
                        % % Finally mMTC allocation
                        % for i = 1:C.N_RB
                        %     if total_allocation(i) == 0 && (dl_allocation_mmtc(i) == 1 || dl_energy_allocation(i, 3) > 0)
                ...
                if strcmp(algorithms{alg_idx}, 'Advanced DART-PRB') && isfield(rl_state, 'outage_history')
                    ...
                    % Update learned adjustments based on fairness
                    fairness = calculateJainsFairnessIndex(dl_slicing_ratios);
                    target_fairness = 0.85;
                    fairness_error = target_fairness - fairness;
                    
                    % Apply corrective adjustments to improve fairness
                    if fairness_error > 0.1
                        % Find which service has the highest allocation
                        [max_val, max_idx] = max(dl_slicing_ratios);
                        [min_val, min_idx] = min(dl_slicing_ratios);
                        
                        % Adjust the allocations to improve fairness
                        if max_idx == 1
                            rl_state.learned_adjustment.v2x = -0.05;
                        elseif max_idx == 2
                            rl_state.learned_adjustment.embb = -0.05;
                        else
                            rl_state.learned_adjustment.mmtc = -0.05;
                        end
            ...
            if strcmp(algorithms{alg_idx}, 'Advanced DART-PRB') || strcmp(algorithms{alg_idx}, 'Basic DART-PRB')
                ...
                if strcmp(algorithms{alg_idx}, 'Advanced DART-PRB')
                    ...
                    sla_violations(round, alg_idx) = checkSLAViolationsEnhanced(avg_dl_outage, avg_ul_outage, dl_service_demands_sum, ul_service_demands_sum);
            ...
            if round < num_rounds
                try
                    if strcmp(algorithms{alg_idx}, 'Advanced DART-PRB')
                        % Use the fair slicing allocation instead of DQN
                        [dl_slicing_ratios, ul_slicing_ratios, rl_state] = updateWithFairSlicingAllocation(rl_state, dl_service_demands_sum, ul_service_demands_sum, round);
                    elseif strcmp(algorithms{alg_idx}, 'Basic DART-PRB')
...
function skip = shouldSkipComputation(round, drop)
    % Determine if we should skip expensive computation for efficiency
    
    % Skip every other round for major computations
    if mod(round, 2) == 0 && mod(drop, 2) == 0
        skip = true;
    else
        skip = false;
    end
