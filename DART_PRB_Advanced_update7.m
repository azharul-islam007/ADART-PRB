function DART_PRB_Advanced_update7()
...
function DART_PRB_Enhanced_Simulation()
    ...
    for alg_idx = 1:num_algorithms
        ...
        for round = 1:num_rounds
            ...
            % Calculate separate fairness indices for DL and UL with proper ratios
...
function [dl_allocation_v2x, dl_allocation_embb, dl_allocation_mmtc] = reallocateUnusedResources(network, dl_allocation_v2x, dl_allocation_embb, dl_allocation_mmtc)
    % Calculate actual usage rates for each service type
    total_prbs = length(dl_allocation_v2x);
    
    % Get v2x usage - either from network stats or estimate
    if isfield(network, 'v2x_usage_stats') && isfield(network.v2x_usage_stats, 'utilization')
        v2x_usage_rate = network.v2x_usage_stats.utilization;
    else
        % Estimate from vehicle activity
        if isfield(network, 'veh_UEs') && isfield(network.veh_UEs, 'packets')
            active_v2x = sum(network.veh_UEs.packets > 0) / max(1, length(network.veh_UEs.packets));
            v2x_usage_rate = min(1, active_v2x * 1.2); % Slightly higher estimate
        else
            v2x_usage_rate = 0.8; % Reasonable default
        end
    end
    
    % Get eMBB usage rate
    if isfield(network, 'embb_usage_stats') && isfield(network.embb_usage_stats, 'utilization')
        embb_usage_rate = network.embb_usage_stats.utilization;
    else
        % Estimate from eMBB activity
        if isfield(network, 'eMBB_UEs') && isfield(network.eMBB_UEs, 'sessions')
            active_embb = sum(network.eMBB_UEs.sessions > 0) / max(1, length(network.eMBB_UEs.sessions));
            embb_usage_rate = min(1, active_embb * 1.2);
        else
            embb_usage_rate = 0.8; % Reasonable default
        end
    end
    
    % Get mMTC usage rate
    if isfield(network, 'mmtc_usage_stats') && isfield(network.mmtc_usage_stats, 'utilization')
        mmtc_usage_rate = network.mmtc_usage_stats.utilization;
    else
        % Estimate from mMTC activity
        if isfield(network, 'mMTC_UEs') && isfield(network.mMTC_UEs, 'tx_flags')
            active_mmtc = sum(network.mMTC_UEs.tx_flags > 0) / max(1, length(network.mMTC_UEs.tx_flags));
            mmtc_usage_rate = min(1, active_mmtc * 1.2);
        else
            mmtc_usage_rate = 0.6; % Reasonable default
        end
    end
    
    % Calculate actual vs. allocated PRBs
    v2x_allocated = sum(dl_allocation_v2x);
    embb_allocated = sum(dl_allocation_embb);
    mmtc_allocated = sum(dl_allocation_mmtc);
    ...
    % Calculate transfer capacity - different borrowing limits for each service
    v2x_borrowable = min(v2x_unused, round(v2x_allocated * 0.2)); % Allow borrowing up to 20% of v2x allocation
    embb_borrowable = min(embb_unused, round(embb_allocated * 0.3)); % Allow borrowing up to 30% of embb allocation
    mmtc_borrowable = min(mmtc_unused, round(mmtc_allocated * 0.5)); % Allow borrowing up to 50% of mmtc allocation
    
    total_borrowable = v2x_borrowable + embb_borrowable + mmtc_borrowable;
    
    % If no borrowable resources or no need, exit
    if total_borrowable == 0 || total_need == 0
        return;
    end
    ...
    if total_need > 0
        ...
        v2x_borrow_share = 1/3;
        embb_borrow_share = 1/3;
        mmtc_borrow_share = 1/3;
    end
    ...
    dl_allocation_v2x(v2x_transfer_out) = 0;
    dl_allocation_embb(embb_transfer_out) = 0;
    dl_allocation_mmtc(mmtc_transfer_out) = 0;
    
    % Then redistribute
    if v2x_transfer_count > 0
        v2x_share = all_transfers(1:v2x_transfer_count);
        dl_allocation_v2x(v2x_share) = 1;
    end
    
    if embb_transfer_count > 0
        embb_share = all_transfers(v2x_transfer_count+1:v2x_transfer_count+embb_transfer_count);
        dl_allocation_embb(embb_share) = 1;
    end
    
    if mmtc_transfer_count > 0
        mmtc_share = all_transfers(v2x_transfer_count+embb_transfer_count+1:end);
        dl_allocation_mmtc(mmtc_share) = 1;
    end
...
function [dl_allocation_v2x, dl_allocation_embb, dl_allocation_mmtc, ul_allocation_v2x, ul_allocation_embb, ul_allocation_mmtc] = optimizedResourceAllocationWithSpatialReuse(network, dl_slicing_ratios, ul_slicing_ratios)
    ...
    % Enhanced interference thresholds for more aggressive spatial reuse
    interference_threshold_v2x = 0.25;  % Conservative for V2X (reliability critical)
    interference_threshold_embb = 0.40; % Aggressive for eMBB (can handle more)
    interference_threshold_mmtc = 0.45; % Very aggressive for mMTC (less sensitive)
    ...
    if isfield(network, 'interference_cancellation_capability')
        ...
        interference_threshold_v2x = interference_threshold_v2x * (1 + cancellation_factor * 0.5);
        interference_threshold_embb = interference_threshold_embb * (1 + cancellation_factor);
        interference_threshold_mmtc = interference_threshold_mmtc * (1 + cancellation_factor * 1.5);
    end
    ...
    % Calculate more aggressive PRBs per cluster with improved spatial reuse
    spatial_reuse_factor_v2x = 1.2;  % Increased from original factor
    ...
    % Similar implementation for eMBB and mMTC
    % Would follow same pattern with different spatial reuse factors and thresholds
    
    % Check for any unallocated PRBs and assign them dynamically
    unallocated_prbs = find(dl_allocation_v2x == 0 & dl_allocation_embb == 0 & dl_allocation_mmtc == 0);
    
    if ~isempty(unallocated_prbs)
        ...
        if total_demand > 0
            ...
            num_unallocated = length(unallocated_prbs);
            ...
            dl_allocation_v2x(unallocated_prbs(1:v2x_extra)) = 1;
            dl_allocation_embb(unallocated_prbs(v2x_extra+1:v2x_extra+embb_extra)) = 1;
            dl_allocation_mmtc(unallocated_prbs(v2x_extra+embb_extra+1:end)) = 1;
        end
    end
    
    % Copy DL allocations to UL for simplicity
    % In a complete implementation, you would apply the same logic to UL
...
function [dl_slicing_ratios, ul_slicing_ratios, new_rl_state] = updateWithAdaptiveAllocation(rl_state, dl_service_demands_sum, ul_service_demands_sum, round, current_utilization)
    ...
    % Dynamic fairness-utilization balancing based on current utilization
    dl_utilization_threshold = 0.70; % Target utilization threshold for DL
    ul_utilization_threshold = 0.68; % Slightly lower target for UL due to constraints
    
    % Extract separate DL and UL utilization if available
    dl_current_utilization = current_utilization;
    ul_current_utilization = current_utilization;
    ...
    % Separate fairness weights for DL and UL based on current utilization
    % DL fairness weight selection
    if dl_current_utilization < 0.65
        fairness_weight_dl = 0.25; % Lower fairness weight when utilization is low
    elseif dl_current_utilization < dl_utilization_threshold
        fairness_weight_dl = 0.35; % Moderate fairness weight
    else
        fairness_weight_dl = 0.45; % Higher fairness weight when utilization is good
    end
    
    % UL fairness weight selection (higher baseline for better fairness)
    if ul_current_utilization < 0.65
        fairness_weight_ul = 0.30; % Lower but still higher than DL
    elseif ul_current_utilization < ul_utilization_threshold
        fairness_weight_ul = 0.40; % Moderate fairness weight
    else
        fairness_weight_ul = 0.50; % Higher fairness weight when utilization is good
    end
    ...
    % If fairness is too low, apply fairness correction to DL
    if dl_current_fairness < 0.75
        % Find min and max allocation
        [min_val, min_idx] = min(dl_temp_ratios);
        [max_val, max_idx] = max(dl_temp_ratios);
        
        % Redistribute a small amount (up to 5%)
        redistribution = min(0.05, (max_val - min_val) * 0.3);
    ...
    % Apply fairness correction to UL (more aggressively)
    if ul_current_fairness < 0.78
        % Find min and max allocation
        [min_val, min_idx] = min(ul_temp_ratios);
        [max_val, max_idx] = max(ul_temp_ratios);
        
        % Redistribute a slightly larger amount for UL
        redistribution = min(0.06, (max_val - min_val) * 0.35);
    ...
    if isfield(new_rl_state, 'traffic_history') && ...
       ...
       ~isempty(new_rl_state.traffic_history.v2x)
        ...
        if total_avg > 0
            ...
            dl_v2x_min = max(dl_min_floor, min(0.3, 0.5 * avg_v2x / total_avg));
            dl_embb_min = max(dl_min_floor, min(0.3, 0.5 * avg_embb / total_avg));
            dl_mmtc_min = max(dl_min_floor, min(0.3, 0.5 * avg_mmtc / total_avg));
            
            % Set slightly different UL minimums
            ul_v2x_min = max(ul_min_floor, min(0.3, 0.5 * avg_v2x / total_avg));
            ul_embb_min = max(ul_min_floor, min(0.3, 0.5 * avg_embb / total_avg));
            ul_mmtc_min = max(ul_min_floor, min(0.3, 0.5 * avg_mmtc / total_avg));
        end
    end
    ...
    % Apply maximum caps (different for DL and UL)
    dl_max_cap = 0.45;
    if dl_current_utilization < 0.65
        dl_max_cap = 0.55;
    end
    
    ul_max_cap = 0.42; % Slightly lower max for UL to promote fairness
    if ul_current_utilization < 0.65
        ul_max_cap = 0.52; % Slightly lower max for UL to promote fairness
    end
    ...
    if isfield(new_rl_state, 'outage_history')
        dl_outage_adjustment = 0.03;
        ul_outage_adjustment = 0.025; % Slightly less aggressive for UL
    ...
    if ~isfield(new_rl_state, 'dl_fairness_history')
        ...
        new_rl_state.fairness_target = 0.80;
    end
    ...
    if length(new_rl_state.dl_fairness_history) > 5
        ...
        if dl_avg_fairness < new_rl_state.fairness_target && dl_current_utilization > 0.65
            ...
            dl_v2x_ratio = 0.7 * dl_v2x_ratio + 0.3 * (1/3);
            dl_embb_ratio = 0.7 * dl_embb_ratio + 0.3 * (1/3);
            dl_mmtc_ratio = 0.7 * dl_mmtc_ratio + 0.3 * (1/3);
        end
        
        if ul_avg_fairness < new_rl_state.fairness_target && ul_current_utilization > 0.65
            % Adjust UL ratios toward equal allocation (more aggressively)
            ul_v2x_ratio = 0.65 * ul_v2x_ratio + 0.35 * (1/3);
            ul_embb_ratio = 0.65 * ul_embb_ratio + 0.35 * (1/3);
            ul_mmtc_ratio = 0.65 * ul_mmtc_ratio + 0.35 * (1/3);
        end
    end
...
function slicing_ratios = validateSlicingRatios(slicing_ratios, current_utilization)
    ...
    % Dynamically adjust minimum constraints based on utilization
    if nargin > 1 && ~isempty(current_utilization)
        if current_utilization < 0.6
            ...
            min_ratio = 0.10;
            max_ratio = 0.60; % Allow more flexible allocation to boost utilization
