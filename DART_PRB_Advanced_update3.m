function DART_PRB_Advanced_update3()
            ...
            for drop = 1:drops_per_round
                ...
                try
                    ...
                    elseif strcmp(algorithms{alg_idx}, 'Basic DART-PRB')
                        ...
                        ul_slicing_ratios = [sum(ul_allocation_v2x)/C.N_RB; sum(ul_allocation_embb)/C.N_RB; sum(ul_allocation_mmtc)/C.N_RB];
                    elseif strcmp(algorithms{alg_idx}, 'RL-based Allocation')
                        % For RL-based, just use the current ratios to derive the allocations
                        ...
                        % Sequential allocation based on slicing ratios
                        v2x_count = round(dl_slicing_ratios(1) * C.N_RB);
                        embb_count = round(dl_slicing_ratios(2) * C.N_RB);
                        mmtc_count = C.N_RB - v2x_count - embb_count;
                        
                        dl_allocation_v2x(1:v2x_count) = 1;
                        dl_allocation_embb(v2x_count+1:v2x_count+embb_count) = 1;
                        dl_allocation_mmtc(v2x_count+embb_count+1:C.N_RB) = 1;
                        
                        % Same for UL
                        v2x_count = round(ul_slicing_ratios(1) * C.N_RB);
                        embb_count = round(ul_slicing_ratios(2) * C.N_RB);
                        mmtc_count = C.N_RB - v2x_count - embb_count;
                        
                        ul_allocation_v2x(1:v2x_count) = 1;
                        ul_allocation_embb(v2x_count+1:v2x_count+embb_count) = 1;
                        ul_allocation_mmtc(v2x_count+embb_count+1:C.N_RB) = 1;
                    elseif strcmp(algorithms{alg_idx}, 'Static Equal Allocation')
                        % Static Equal Allocation - each service gets 1/3 of PRBs
                        v2x_count = round(C.N_RB / 3);
                        embb_count = round(C.N_RB / 3);
                        mmtc_count = C.N_RB - v2x_count - embb_count;
                        ...
                        dl_slicing_ratios = [v2x_count/C.N_RB; embb_count/C.N_RB; mmtc_count/C.N_RB];
                        ul_slicing_ratios = dl_slicing_ratios;
                    elseif strcmp(algorithms{alg_idx}, 'Traffic-based Allocation')
                        % Traffic-based Allocation - based on current service demands
                        % First calculate service demands
                        [temp_util, service_demands] = calculatePRBUtilization(network, [1/3; 1/3; 1/3], 'dl');
                        ...
                        if total_demand > 0
                            v2x_ratio = max(0.1, v2x_demand / total_demand);
                            embb_ratio = max(0.1, embb_demand / total_demand);
                            mmtc_ratio = max(0.1, mmtc_demand / total_demand);
                        ...
                        % Allocate PRBs based on these ratios
                        v2x_count = round(v2x_ratio * C.N_RB);
                        embb_count = round(embb_ratio * C.N_RB);
                        mmtc_count = C.N_RB - v2x_count - embb_count;
...
function [dl_allocation_v2x, dl_allocation_embb, dl_allocation_mmtc, ul_allocation_v2x, ul_allocation_embb, ul_allocation_mmtc] = allocateResourcesForAllAlgorithms(algorithm_name, network, dl_slicing_ratios, ul_slicing_ratios, C)
    ...
    % Apply algorithm-specific optimizations if needed
    if strcmpi(algorithm_name, 'Advanced DART-PRB')
        % For Advanced DART-PRB, we can still use the optimized algorithms
        % but only if the current allocation is valid and non-empty
        if all(isfinite(dl_allocation_v2x)) && all(isfinite(dl_allocation_embb)) && all(isfinite(dl_allocation_mmtc))
            try
                % Try optimized allocation but keep current as fallback
                [opt_dl_v2x, opt_dl_embb, opt_dl_mmtc, opt_ul_v2x, opt_ul_embb, opt_ul_mmtc] = ...
                    optimizedResourceAllocationWithSpatialReuse(network, dl_slicing_ratios, ul_slicing_ratios);
                
                % Verify the optimization worked properly
                if sum(opt_dl_v2x) > 0 && sum(opt_dl_embb) > 0 && sum(opt_dl_mmtc) > 0 && ...
                   sum(opt_ul_v2x) > 0 && sum(opt_ul_embb) > 0 && sum(opt_ul_mmtc) > 0
                    % Use the optimized allocation
                    dl_allocation_v2x = opt_dl_v2x;
                    dl_allocation_embb = opt_dl_embb;
                    dl_allocation_mmtc = opt_dl_mmtc;
                    ul_allocation_v2x = opt_ul_v2x;
                    ul_allocation_embb = opt_ul_embb;
                    ul_allocation_mmtc = opt_ul_mmtc;
                end
            catch
                % If optimization fails, we keep the basic allocation (already set)
            end
        end
    end
    
    % Safety check: ensure no PRB is assigned multiple services
    for i = 1:C.N_RB
...
% Extra safe implementation of for robustness
