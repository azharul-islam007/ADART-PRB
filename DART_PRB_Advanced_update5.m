function DART_PRB_Advanced_update5()
...
function [dl_slicing_ratios, ul_slicing_ratios, new_rl_state] = updateWithAdaptiveAllocation(rl_state, dl_service_demands_sum, ul_service_demands_sum, round, current_utilization)
    ...
    % Apply adaptive fairness component with higher weights
    if current_utilization < 0.7
        fairness_weight = 0.4; % Increased from 0.2 - prioritize fairness more when utilization is low
    elseif current_utilization < 0.85
        fairness_weight = 0.3; % Increased from previous value
    else
        fairness_weight = 0.4; % Increased to maintain fairness even at high utilization
    end
    
    % Calculate weighted ratios with higher weight on equal distribution
    ...
    % Add a fairness boost step before normalization
    ...
    % If fairness is too low, adjust ratios
    ...
    v2x_min = 0.15;  % Increased minimum from 0.2 to 0.15
    embb_min = 0.15; % Increased from 0.2 to 0.15
    mmtc_min = 0.15; % Increased from 0.2 to 0.15
    ...
    if isfield(new_rl_state, 'traffic_history') && ...
       ...
       ~isempty(new_rl_state.traffic_history.v2x)
        ...
        if total_avg > 0
            % Set minimums proportional to average demands, but with a higher floor
            v2x_min = max(0.15, min(0.3, 0.5 * avg_v2x / total_avg)); % Less aggressive minimum
            embb_min = max(0.15, min(0.3, 0.5 * avg_embb / total_avg)); 
            mmtc_min = max(0.15, min(0.3, 0.5 * avg_mmtc / total_avg));
        end
    end
    ...
    % Apply maximum caps to prevent any service from monopolizing resources
    v2x_ratio = min(0.45, v2x_ratio);
    embb_ratio = min(0.45, embb_ratio);
    mmtc_ratio = min(0.45, mmtc_ratio);
    
    % Adjust based on outage history if available
    if isfield(new_rl_state, 'outage_history')
        outage_adjustment = 0.03;  % Reduced from 0.05 to less aggressively adjust for outages
    ...
    if ~isfield(new_rl_state, 'fairness_history')
        ...
        new_rl_state.fairness_target = 0.80; % Target fairness index
    end
    ...
    if length(new_rl_state.fairness_history) > 5
        ...
        if avg_fairness < new_rl_state.fairness_target
            % Adjust ratios toward equal allocation
...
function slicing_ratios = validateSlicingRatios(slicing_ratios)
    ...
    % Ensure non-negative values with higher minimum thresholds for fairness
    slicing_ratios = max(0.15, slicing_ratios); % Increased from 0.05 to 0.15
    
    % Add maximum caps to prevent any service from monopolizing
    slicing_ratios = min(0.45, slicing_ratios); % New cap at 45% for fairness
